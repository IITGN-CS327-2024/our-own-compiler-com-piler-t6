...
class ASTTransformer(Transformer):
    ...
    def functiondecl(self, items):
        ...
        args = items[3]
        return_type = items[6].children[0].value
        body = items[7]
        ...
        try:
            if isinstance(node, int):
                ...
                if node['type'] in ['logicalexpr', 'comparisonexpr']:
                    ...
                    if node['left'].type != 'VARIABLENAME':
    ...
    def comparisonexpr(self, items):
        if len(items) == 1:
            ...
            for i in range(1, len(items), 2):
                operator = items[i]
                right_expr = items[i + 1]
                expr = {"type": "comarisonexpr", "operator": operator, "left": expr, "right": right_expr}
            return self.evaluate_expressionlc(expr)
...
"""
    ...
    if ast:
        output_path = 'AST/arithmetic-test.json'
